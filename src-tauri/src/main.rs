...
/// Sidecars land next to the shell executable in every bundle format:
/// Contents/MacOS on macOS, the install directory on Windows, usr/bin in an
/// AppImage.
    ...
    tauri::Builder::default()
        ...
        .on_window_event(|window, event| {
            if let tauri::WindowEvent::Destroyed = event {
                ...
                // explicitly here or model weights stay resident in RAM.
                #[cfg(unix)]
                {
                    let _ = Command::new("pkill").args(["-f", "llama-server"]).status();
                }
                #[cfg(windows)]
                {
